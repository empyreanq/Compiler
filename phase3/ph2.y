...
program:	stmt_list{printf("\"Grammar\": program | program stmt\n");}
			|{$$=NULL;printf("\"Grammar\": program | empty\n");}
			;
...
stmt:		expr SEMICOLON {resettemp();$$=make_stmt();if(libflag==-2)printf("ERROR :::::: Variable with name : %s is a libfunc!!!!!\nfailed at line : %d\n",libflagname,yylineno);libflag=0;printf("\"Grammar\": stmt | expr SEMICOLON\n");}
...
expr:	 	assignexpr {printf("\"Grammar\": expr | assignexpr\n");}
			...
			| expr GREATER expr {
				$$=newexpr(boolexpr_e);
				$$->sym=newtemp(scope);
				emit(if_greater,$1,$3,$$,currQuad+4,yylineno);
			...
			| expr GREATEREQ expr {
				$$=newexpr(boolexpr_e);
				$$->sym=newtemp(scope);
				emit(if_greatereq,$1,$3,$$,currQuad+4,yylineno);
				emit(assign,newexpr_constbool(0),NULL,$$,0,yylineno);
				emit(jump,NULL,NULL,NULL,currQuad+3,yylineno);
				emit(assign,newexpr_constbool(1),NULL,$$,0,yylineno);
			...
			| expr LESS expr {
				$$=newexpr(boolexpr_e);
				$$->sym=newtemp(scope);
				emit(if_less,$1,$3,$$,currQuad+4,yylineno);
				emit(assign,newexpr_constbool(0),NULL,$$,0,yylineno);
				emit(jump,NULL,NULL,NULL,currQuad+3,yylineno);
				emit(assign,newexpr_constbool(1),NULL,$$,0,yylineno);
			...
			| expr LESSEQ expr {
				$$=newexpr(boolexpr_e);
				$$->sym=newtemp(scope);
				emit(if_lesseq,$1,$3,$$,currQuad+4,yylineno);
				emit(assign,newexpr_constbool(0),NULL,$$,0,yylineno);
				emit(jump,NULL,NULL,NULL,currQuad+3,yylineno);
				emit(assign,newexpr_constbool(1),NULL,$$,0,yylineno);
			...
			| expr EQUAL expr {
				$$=newexpr(boolexpr_e);
				$$->sym=newtemp(scope);
				emit(if_eq,$1,$3,$$,currQuad+4,yylineno);
				emit(assign,newexpr_constbool(0),NULL,$$,0,yylineno);
				emit(jump,NULL,NULL,NULL,currQuad+3,yylineno);
				emit(assign,newexpr_constbool(1),NULL,$$,0,yylineno);
			...
			| expr NOTEQUAL expr {
				$$=newexpr(boolexpr_e);
				$$->sym=newtemp(scope);
				emit(if_noteq,$1,$3,$$,currQuad+4,yylineno);
				emit(assign,newexpr_constbool(0),NULL,$$,0,yylineno);
				emit(jump,NULL,NULL,NULL,currQuad+3,yylineno);
				emit(assign,newexpr_constbool(1),NULL,$$,0,yylineno);
			...
			| term {$$=$1;printf("\"Grammar\": expr | term\n");}
			;
