...
expr:	 	assignexpr {$$=$1;printf("\"Grammar\": expr | assignexpr\n");}
			...
			| expr AND {
				...
				if($1->truelist==NULL){
					...
					emit(if_eq,$1,newexpr_constbool(1),$1,0,yylineno);
					emit(jump,NULL,NULL,NULL,0,yylineno);
				}
			} M expr{
				if($5->truelist==NULL){
					$5->truelist=makelist(currQuad);
					$5->falselist=makelist(currQuad+1);
					emit(if_eq,$5,newexpr_constbool(1),$5,0,yylineno);
					emit(jump,NULL,NULL,NULL,0,yylineno);			
				}
				$$=newexpr(boolexpr_e);
				//$$->sym=newtemp(scope);
				...
				$$->falselist=mergeTrueORFalseList($1->falselist,$5->falselist);
			...
			| expr OR  {
				...
				$$->truelist=mergeTrueORFalseList($1->truelist,$5->truelist);
...
term: 	 	LEFT_PAR {parenthesisbool=1;} expr RIGHT_PAR {$$=$3;parenthesisbool=0; printf("\"Grammar\": term | LEFT_PAR expr RIGHT_PAR\n");}
			...
			|NOT expr {
				$$=newexpr(boolexpr_e);
				if($2->truelist==NULL){
					$2->truelist=makelist(currQuad);
					$2->falselist=makelist(currQuad+1);
					emit(if_eq,$2,newexpr_constbool(1),$2,0,yylineno);
					emit(jump,NULL,NULL,NULL,0,yylineno);
				}
				$$->sym=$2->sym;
...
assignexpr: lvalue ASSIGN expr {
				...
				if($1->type == tableitem_e){
					...
					if(merikicounter==1&&$3->type==boolexpr_e){
						assignexp=$1;
						$$=$3;
					}else{
...
whilecond:		LEFT_PAR expr RIGHT_PAR{
					if(merikicounter==1){
						struct expr* helper=lvalue_expr(newtemp(scope));
						emit(assign,newexpr_constbool(1),NULL,helper,0,yylineno);
						backpatch($2->truelist,currQuad);
						emit(jump,NULL,NULL,NULL,currQuad+3,yylineno);
						emit(assign,newexpr_constbool(0),NULL,helper,0,yylineno);
						backpatch($2->falselist,currQuad);
						if(assignexp){
							emit(assign,helper,NULL,assignexp,0,yylineno);
							helper=lvalue_expr(newtemp(scope));
							emit(assign,assignexp,NULL,helper,0,yylineno);
...
forprefix:	FOR LEFT_PAR elist SEMICOLON M expr SEMICOLON
			{
				...
				if(merikicounter==1){
						...
						struct expr* helper=lvalue_expr(newtemp(scope));
						emit(assign,newexpr_constbool(1),NULL,helper,0,yylineno);
						backpatch($6->truelist,currQuad);
						emit(jump,NULL,NULL,NULL,currQuad+3,yylineno);
						emit(assign,newexpr_constbool(0),NULL,helper,0,yylineno);
						backpatch($6->falselist,currQuad);
						if(assignexp){
							emit(assign,helper,NULL,assignexp,0,yylineno);
							helper=lvalue_expr(newtemp(scope));
							emit(assign,assignexp,NULL,helper,0,yylineno);
				...
				}else emit(if_eq,$6,newexpr_constbool(1),NULL,0,yylineno);
				merikicounter=0;
				assignexp=NULL;
			}
